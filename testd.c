...
int get_door_key(void *s, mf_version *v, mf_key_t *key, unsigned char **uout) {
	zmq_msg_t m;
	...
	if(resp[0] == 't' && zmq_msg_size(&m) >= 18) {
		memcpy(*key, resp+1, 16);

		*uout = strndup(resp+17, zmq_msg_size(&m)-17);
...
int main(int argc, char **argv) {
	...
	if(!(reader = getenv("PCSC_READER"))) {
		reader=pcsc_get_reader(pcsc_ctx);
	}
	...
	mf_interface *intf = mf_interface_new(pcsc_send,(void*)&state);
		...
		ret = mf_get_version(intf, &v);
	        if(ret != MF_OK) {
			fprintf(stderr, "mf_get_version failed: %s\n", mf_error_str(ret));
			goto out;
		}
	        ...
	        mf_key_t door_key;
		unsigned char *user;
	        if(get_door_key(zmq_keystore, &v, &door_key, &user) < 0)
		        goto out;
			...
			snprintf(buf, 128, "./sm-patch/sm -u -t 1 'Hallo %s!'", user);
